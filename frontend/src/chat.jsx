...
function ChatThreadView({ thread, onBack, compact = false, embedded = false, inCall = false, navigate, onThreadActivity, onOpenWhiteboard, markReadOnView = true, onUnreadMessage, onMarkedRead, threads = [] }) {
  const [messages, setMessages] = React.useState([]); const [loading, setLoading] = React.useState(true); const [loadingMore, setLoadingMore] = React.useState(false); const [hasMore, setHasMore] = React.useState(false); const [error, setError] = React.useState(''); const [sending, setSending] = React.useState(false); const [editBusy, setEditBusy] = React.useState(false); const [editingId, setEditingId] = React.useState(null); const [replyTo, setReplyTo] = React.useState(null); const [forwardMsg, setForwardMsg] = React.useState(null); const [forwardBusy, setForwardBusy] = React.useState(false); const [wsState, setWsState] = React.useState('connecting'); const [pendingDeleteId, setPendingDeleteId] = React.useState(null);
  ...
  const token = window.getAccessToken(); const myId = window.currentUserPublicId ? window.currentUserPublicId(token) : String((token ? window.parseJwtPayload(token) : {})?.public_id || (token ? window.parseJwtPayload(token) : {})?.user_id || ''); const other = thread?.other_participant;
  ...
  const deleteMessage = async (id) => { if (!id) return; setError(''); try { const msg = await window.fetchApiJson(`/api/communication/chat/messages/${encodeURIComponent(id)}/`, { method: 'DELETE', auth: true }); setMessages((old) => upsertChatMessage(old, msg)); notifyActivity(msg); } catch (e) { setError(e.message || 'Не удалось удалить'); } finally { setPendingDeleteId(null); } };
  ...
  return <div className={`flex min-h-0 flex-col ${compact || embedded ? 'h-full' : 'h-[min(72vh,760px)]'} ${embedded ? 'bg-[#0a1020]' : 'bg-paper'}`}>
    ...
    {window.ConfirmDialog && (
      <window.ConfirmDialog
        ...
        cancelLabel="Отмена"
        danger
        onConfirm={() => deleteMessage(pendingDeleteId)}
...
function MessagesPage({ navigate, hashParams }) {
  ...
  const targetUser = hashParams?.get('user') || null; const targetCourse = hashParams?.get('course') || null; const token = window.getAccessToken();
