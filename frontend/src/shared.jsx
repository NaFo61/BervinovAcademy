...
function useHashRoute() {
  const [route, setRoute] = React.useState(() => (location.hash.replace('#/', '') || Routes.LANDING));
  React.useEffect(() => {
    const onHash = () => setRoute(location.hash.replace('#/', '') || Routes.LANDING);
    window.addEventListener('hashchange', onHash);
    return () => window.removeEventListener('hashchange', onHash);
  }, []);
  const navigate = (r) => { location.hash = '#/' + r; window.scrollTo({ top: 0, behavior: 'smooth' }); };
...
const I = {
  Logo: ({ className = 'w-8 h-8' }) => (
    <svg viewBox="0 0 32 32" className={className} fill="none">
      ...
      <path d="M13.6 11.7v3.1h2.8c1.3 0 2-.5 2-1.6s-.7-1.5-2-1.5h-2.8zM13.6 17.2v3.5h3.1c1.4 0 2.2-.6 2.2-1.8 0-1.1-.8-1.7-2.2-1.7h-3.1z" fill="#7C3AED"/>
    </svg>
  ),
  Search: ({ className }) => (
...
const COURSES = [
  {
    ...
    gradFrom: '#7C3AED', gradTo: '#F97316', accentEmoji: 'Py', popularity: 92,
  },
  {
    ...
    gradFrom: '#6D28D9', gradTo: '#EC4899', accentEmoji: '∑', popularity: 86,
  },
  {
    ...
    gradFrom: '#06B6D4', gradTo: '#7C3AED', accentEmoji: '⌘', popularity: 98,
  },
  {
    ...
    gradFrom: '#F97316', gradTo: '#EAB308', accentEmoji: 'ML', popularity: 74,
  },
  {
    ...
    gradFrom: '#0EA5E9', gradTo: '#22C55E', accentEmoji: 'SQL', popularity: 68,
  },
  {
    ...
    gradFrom: '#7C3AED', gradTo: '#22D3EE', accentEmoji: 'SD', popularity: 81,
  },
];
      ...
      <div className="glass border-b border-black/5">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 h-16 flex items-center gap-6">
          <button onClick={() => navigate(Routes.LANDING)} className="flex items-center gap-2.5 group">
            <I.Logo className="w-9 h-9 drop-shadow-[0_4px_12px_rgba(124,58,237,0.4)] group-hover:scale-105 transition-transform" />
          ...
          <button onClick={() => navigate(Routes.AUTH)} className="hidden sm:inline-flex h-10 px-3 items-center text-sm font-medium text-ink/70 hover:text-ink">
            Войти
          </button>
          <button onClick={() => navigate(Routes.AUTH)} className="btn-grad btn-shimmer h-10 px-4 rounded-xl text-white text-sm font-semibold shadow-soft">
            Начать учиться
          </button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 py-14 grid grid-cols-2 md:grid-cols-5 gap-10">
        <div className="col-span-2">
          ...
          <p className="text-sm text-ink/60 max-w-sm">
            Учим программированию вживую. Ментор Мирон смотрит твой код, отвечает на вопросы и не даёт сдаться.
          </p>
        ...
        <FooterCol title="Компания" items={[
          { label: 'О Мироне' }, { label: 'Блог' }, { label: 'Карьера' }, { label: 'Контакты' },
        ]} />
  ...
  return (
    <div className={`relative overflow-hidden ${big ? 'h-44' : 'h-32'} ${className}`}
      ...
      <svg className="absolute inset-0 w-full h-full opacity-25" viewBox="0 0 400 200" preserveAspectRatio="none">
        <defs>
          <pattern id={`grid-${course.id}`} width="32" height="32" patternUnits="userSpaceOnUse">
            <path d="M32 0H0V32" stroke="white" strokeWidth="0.5" fill="none"/>
          </pattern>
        </defs>
        <rect width="400" height="200" fill={`url(#grid-${course.id})`}/>
      </svg>
    ...
    >
      ...
      <div className="p-5 flex flex-col gap-3 flex-1">
        ...
        <div className="flex items-center justify-between mt-2 pt-3 border-t border-black/[0.05]">
          <div className="text-[15px] font-bold">{course.price.toLocaleString('ru-RU')} ₽</div>
...
function FloatingShapes() {
  const shapes = [
    { left: '6%', top: '15%', size: 64, hue: '#7C3AED', delay: 0 },
    { left: '88%', top: '20%', size: 48, hue: '#F97316', delay: 1 },
    { left: '12%', top: '70%', size: 40, hue: '#EC4899', delay: 2 },
    { left: '82%', top: '75%', size: 80, hue: '#7C3AED', delay: 3 },
    { left: '50%', top: '85%', size: 32, hue: '#06B6D4', delay: 1.5 },
  ];
  return (
...
Object.assign(window, {
  Routes, useHashRoute, I, TopNav, Footer, CourseCover, CourseCard, COURSES, CATEGORIES, FloatingShapes, FM
});
