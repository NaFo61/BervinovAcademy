...
def test_mentor_role_sets_staff_only():
    user = make_user(email="m@example.com", phone="+79006667788", role="mentor")
...
def test_changing_role_from_mentor_to_student_resets_flags():
    user = make_user(email="change@example.com", phone="+79008880000", role="mentor")
    assert user.is_staff
...
def test_clean_passes_with_email_only():
    user = User(first_name="X", last_name="Y", email="ok@example.com", role="student")
    # Не должно бросать исключение
    user.clean()


def test_clean_passes_with_phone_only():
    user = User(first_name="X", last_name="Y", phone="+79000000099", role="student")
    user.clean()
...
def test_clean_admin_without_email_raises():
    user = User(first_name="A", last_name="B", phone="+79008889900", role="admin")
...
def test_clean_admin_without_phone_raises():
    user = User(first_name="A", last_name="B", email="admin@example.com", role="admin")
    with pytest.raises(ValidationError) as exc_info:
        user.clean()
...
def test_clean_superuser_flag_also_requires_email():
    user = User(
        ...
        phone="+79008889901", role="student",
        is_superuser=True,
    )
    with pytest.raises(ValidationError) as exc_info:
        user.clean()
...
def test_clean_duplicate_email_raises():
    ...
    duplicate = User(first_name="B", last_name="C", email="dup@example.com", role="student")
    with pytest.raises(ValidationError) as exc_info:
        duplicate.clean()
    assert "email" in exc_info.value.message_dict
...
def test_clean_duplicate_phone_raises():
    ...
    duplicate = User(first_name="B", last_name="C", phone="+79009990011", role="student")
    with pytest.raises(ValidationError) as exc_info:
        duplicate.clean()
    assert "phone" in exc_info.value.message_dict
